    ...
    <div class="container">
        <div class="page-header">
            <div class="page-header-left">
                ...
                <p>@lang('messages.edit_product_desc')</p>
            </div>
        </div>
            ...
            <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col-md-6 mb-4">
                        ...
                        <input type="text" name="name" class="input-custom" value="{{ old('name', $product->name) }}" required>
                    </div>
                    ...
                    <div class="col-md-6 mb-4">
                        ...
                        <input type="number" step="0.01" name="price" class="input-custom" value="{{ old('price', $product->price) }}" required>
                    </div>

                    <div class="col-md-6 mb-4">
                        <label class="label-custom">@lang('messages.availability')</label>
                        <select name="status" class="select-custom">
                            <option value="1" {{ $product->status == 1 ? 'selected' : '' }}>✅ @lang('messages.in_stock')</option>
                            <option value="0" {{ $product->status == 0 ? 'selected' : '' }}>❌ @lang('messages.out_of_stock')</option>
                        </select>
                    </div>

                    <div class="col-12 mb-4">
                        <label class="label-custom">@lang('messages.description')</label>
                        <textarea name="description" class="input-custom" rows="4" style="height: auto;">{{ old('description', $product->description) }}</textarea>
                    </div>
                    ...
                    <div class="col-12 mb-5">
                        ...
                        <div class="image-upload-wrapper" id="drop-zone" onclick="document.getElementById('imageInput').click()">
                            ...
                            <p class="mb-0 text-muted" id="file-name">@lang('messages.click_to_change_photo')</p>
                            <input type="file" name="image" id="imageInput" hidden accept="image/*">
                        </div>
                        @if ($product->image)
                            <div class="mt-2">
                                <small>@lang('messages.current_image') :</small><br>
                                <img src="{{ asset($product->image) }}" width="100" class="rounded shadow-sm" alt="{{ $product->name }}">
                            </div>
                        @else
                            <div class="mt-2">
                                <small>No image</small><br>
                                <img src="https://images.unsplash.com/photo-1546069901-ba9599a7e63c?auto=format&fit=crop&w=100&q=80" width="100" class="rounded shadow-sm" alt="No image">
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="d-flex align-items-center justify-content-between pt-4 border-top">
                    ...
                    <button type="submit" class="btn-submit">
                        <i class="fas fa-check me-2"></i> @lang('messages.update_product')
                    </button>
                </div>
            </form>
        </div>
